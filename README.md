<div align="center">
  <img src="assets/welcome.svg" alt="Open Source Community Projects" width="600">
  

  <div> <br/></div>
      <img src="https://camo.githubusercontent." alt="Open Source Community Projects" >
      
  [![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat-square)](http://makeapullrequest.com)
  [![License](https://img.shields.io/badge/license-MIT-blue.svg?style=flat-square)](LICENSE)
  [![Join to community](https://img.shields.io/badge/chat-on%20Telegram-blue.svg?style=flat-square)](https://t.me/OpenSource_Chat)

    
  A curated collection of awesome open source projects created by our community members. 
  Join us to share your projects, contribute, and collaborate with fellow developers!
</div>

## 🌟 About

Welcome to the official collection of projects from the [Опенсорсеры community](https://t.me/OpenSource_Chat)! This repository showcases the best open source projects created by our community members.


## 🚀 Quick Navigation

Jump to category:
- [🌐 Web Applications](#web-applications)
- [🛠 Developer Tools](#developer-tools)
- [⚙️ Utilities](#utilities)
- [📱 Mobile Apps](#mobile-apps)
- [🎮 Games & Graphics](#games--graphics)
- [🤖 AI/ML Projects](#aiml-projects)
- [✨ Other](#other)

---
<a href="https://t.me/OpenSource_Chat">
  <img src="assets/join.svg" alt="Open Source Community Projects" width="600">
</a>




## 🌐 Web Applications

### [CMS Витраж](http://gitlab.com/digitalprotopia/cms)
- **Description**: Мультимодальная (многоголовая) CMS. Сочетает в себе функции Wordpress, Tilda, Airtable и конструктора ботов. Позволяет выстраивать произвольное количество сайтов, ботов и других способов взаимодействия с пользователем. Использует современный стек технологий, сайты и виджеты пишут на React, триггеры на javascript, и то и другое пишется в админке без необходимости перекомпиляции кода.
- **Tech Stack**:  PostgreSQL/node.js/Typescript/GraphQL/React
- **Category**: Web Application
- **Maintainer**: [tg@dilesoft](https://t.me/dilesoft)

### [S3app](http://gitlab.com/digitalprotopia/cms)
- **Description**: Система управления горизонтальными (плоскими) организациями и сообществами на основе идей Социократии 3.0. Поддерживает коллективный доступ к управлению задачами, целями, фактами, предложениями и соглашениями. Позволяет гибко выстраивать и перестраивать иерархии зон ответственности и целей.
- **Tech Stack**: PostgreSQL/node.js/Typescript/GraphQL/React
- **Category**: Web Application / Organization management system
- **Maintainer**: [tg@dilesoft](https://t.me/dilesoft)
- **Community**: https://t.me/digitalprotopia

## 🛠 Developer Tools

## 📱 Mobile Apps

## 🎮 Games & Graphics

## 🤖 AI/ML Projects


## ⚙️ Utilities

## 🚀 How to Add Your Project

We'd love to feature your project! Here's how to add it:

1. Fork this repository
2. Add your project to the appropriate category in `README.md`
3. Create a Pull Request with the following format:
   ```markdown
   ### [Project Name](https://github.com/yourusername/yourproject)
   - **Description**: Brief description of your project
   - **Tech Stack**: Main technologies used
   - **Category**: Web/Mobile/Game/AI/Utility
   ```



## 📜 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.


## 🙏 Acknowledgments

- All the amazing contributors who share their work
- The open source community for continuous inspiration
